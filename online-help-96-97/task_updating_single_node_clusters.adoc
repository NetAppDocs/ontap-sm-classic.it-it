...
+
link:task_obtaining_ontap_software_images.md#["Ottenere immagini del software ONTAP"]
...
** Se la convalida viene completata con avvisi, selezionare la casella di controllo *continua aggiornamento con avvisi*, quindi fare clic su *continua*. Una volta completata la convalida e in corso l'aggiornamento, l'aggiornamento potrebbe essere in pausa a causa di errori. È possibile fare clic sul messaggio di errore per visualizzare i dettagli, quindi eseguire le azioni correttive prima di riprendere l'aggiornamento.
