...
** *Mostra in risultati traccia*: Specificare se si desidera visualizzare solo le voci di accesso negato o tutte le voci. Fare clic su *Apply* (Applica) per applicare i dettagli specificati e tornare alla finestra Trace file Access (accesso al file di traccia).
...
** Se è stato specificato il protocollo NFS, viene visualizzata la finestra di dialogo autorizzazioni file effettive, che elenca le autorizzazioni del file associate al file a cui l'utente sta tentando di accedere. Un segno di spunta indica che le autorizzazioni sono concesse e un "`X`" indica che le autorizzazioni non sono concesse.
