...
** ONTAP 9.2 o versioni precedenti: Fare clic sulla scheda inferiore *protezione dati* per identificare il volume di destinazione nella relazione SnapMirror e il nome della SVM che contiene il volume. Viene visualizzato il campo Type (tipo) `Restore` temporaneamente. Al termine dell'operazione di ripristino, viene visualizzato il campo `Vault`.
+
