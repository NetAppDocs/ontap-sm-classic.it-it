...
* Sono disponibili almeno due porte di destinazione FC su ciascun nodo del cluster. Le porte FC e UTA2 integrate (dette anche "`CNA`"), così come alcuni adattatori, sono configurabili. La configurazione di queste porte viene eseguita nell'interfaccia della riga di comando (CLI) di ONTAP e non viene trattata in questa procedura.
